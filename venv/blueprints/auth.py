...
def register():
    ...
    @app.route('/hr/assignemployee', methods=['POST'])
   @jwt_required()
   def assign_employee():
...
def login():
    ...
    access_token = create_access_token(identity=user.id, expires_delta=False)  # Expiration par défaut (configure via config)
